...
def generate_errorbar_plots(simple_rows: list[dict[str, float | str | int]], structured_rows: list[dict[str, float | str | int]], output_plot: Path) -> list[Path]:
    if not simple_rows or not structured_rows:
        return []
    ...
    simple_by_model = {str(row["model_name"]): row for row in simple_rows}
    structured_by_model = {str(row["model_name"]): row for row in structured_rows}
    models = [model for model in simple_by_model if model in structured_by_model]

    if not models:
        return []

    for title, mean_key, std_key, simple_color, structured_color, suffix in metrics:
        simple_means = [float(simple_by_model[model][mean_key]) for model in models]
        simple_errors = [float(simple_by_model[model][std_key]) for model in models]
        structured_means = [float(structured_by_model[model][mean_key]) for model in models]
        structured_errors = [float(structured_by_model[model][std_key]) for model in models]
        ...
        output_file = output_plot.with_name(f"compare_{output_plot.stem}_{suffix}_errorbar{output_plot.suffix}")
        fig.savefig(output_file, dpi=150)
        output_paths.append(output_file)
        plt.close(fig)
...
def main() -> None:
    ...
    simple_results_dir = Path("test1-simple-results")
    structured_results_dir = Path("test1-structure-results")
    output_plot = Path("test1-results/model_performance_summary.png")

    simple_per_model = collect_metrics(simple_results_dir)
    structured_per_model = collect_metrics(structured_results_dir)
    simple_rows = build_summary(simple_per_model)
    structured_rows = build_summary(structured_per_model)
    #print_summary(rows)

    #boxplot_paths = generate_box_plot(simple_per_model, structured_per_model, output_plot)
    #for path in boxplot_paths:
    #    print(f"Box plot written to: {path}")

    errorbar_paths = generate_errorbar_plots(simple_rows, structured_rows, output_plot)
