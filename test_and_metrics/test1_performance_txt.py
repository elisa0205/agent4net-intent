...
def collect_metrics(results_dir: Path, temperature: float) -> dict[str, dict[str, list[float]]]:
    per_model: dict[str, dict[str, list[float]]] = {}

    temp_tag = None
    if temperature is not None:
        # normalize: 0.2 -> "temp_0.2"
        temp_tag = f"temp_{temperature}".replace(",", ".")  # per sicurezza

    for report_file in sorted(results_dir.rglob("*.txt")):
        #print(f"Processing report file: {report_file}")
        if temp_tag is not None and temp_tag not in str(report_file):
            continue
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Temperature value to filter results (optional)",
    )

    args = parser.parse_args()

    per_model = collect_metrics(args.results_dir, args.temperature)
    rows = build_summary(per_model)
    print_summary(rows)

# & c:\Users\39388\Documents\Uni\Tesi\.venv\Scripts\python.exe c:/Users/39388/Documents/Uni/Tesi/test_and_metrics/test1_performance_txt.py --temperature 1.4
